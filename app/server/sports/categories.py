...
main_arr = {
	"passing":[], "rushing":[], "receiving":[], "punting":[],
	"kicking":[], "fumbles":[], "defense":[], "general":[], 'score':[]
}
...
def players_for_team(team):
	for key in variable.keys():
		...
		if part in main_arr.keys():
			...
			if "score" in key:
				insert_key = "score"
			elif part in ["kickret"]:
				insert_key = "kicking"
			elif part in ["puntret"]:
				insert_key = "punting"
			elif key not in ["offense_tds", "offense_yds", "team", "player_id", "index"]:
				insert_key = "general"
		if insert_key != "":
			bisect.insort(main_arr[insert_key], key)
