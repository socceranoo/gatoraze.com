module.exports = function() {
	...
	function rank(str, index, value) {
		this.index = index;
		this.value = value;
	}	
	function suit(str, index) {
		this.index = index;
		this.value = index;
	}	
	...
	cardObject.createCardDeck = function () {
		return masterCardDeck;
	};

	cardObject.createCardSet = function (max) {
		return [];
	};

	cardObject.shuffle = function (cardArr) {
	...
	cardObject.display =  function (cardArr) {
		for (var j = 0 ; j < cardArr.length; j++) {
			console.log(cardArr[j].rank.name+cardArr[j].suit.name);
		}
	};

	cardObject.totalPoints  = function(cardArr, obj){
		var points = 0;
		for (var j = 0 ; j < cardArr.length; j++) {
